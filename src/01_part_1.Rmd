...
4.  Select the desired specifications and configurations for your database, ensuring it meets your requirements. For the sake of this workshop you can just use the Free tier PostgreSQL database instance.

    ![](img/p1/aiven_create_pg_service.gif)

5.  Once the database is created, you will receive the necessary connection details such as host, port, username, and password.

6.  You will use these connection details to establish a connection to your remote PostgreSQL database from your local computer with software tools like QGIS.
...
QGIS will serve as your database client in this workship, enabling seamless connectivity to your database.

If you have another PostgreSQL client of preference (e.g. [pgAdmin](https://www.pgadmin.org/), [Dbeaver](https://dbeaver.io/)), you can also use these database tools to explore your data and understand how databases work.
...
![](img/p1/qgis_add_postgis.gif){width="554"}
...
![](img/p1/qgis_add_connection_details.gif)

Click "Test Connection" to verify the connection and ensure it is successful.

::: hint-box
When using Aiven, ensure first that the database is built:

![](img/p1/aiven_db_running.png){width="317"}
:::

1.  Once the connection is verified, click "OK" to close the dialog and establish the connection.

    1.  If you are using a PostGIS database using Aiven or any other cloud provider the same pop-up window would like this:
    ...
    2.  Using a localhost setup for the workshop, the database connection details would look for example as follows: ![](img/p1/add_postgres.png)

2.  The database should now appear under the "PostgreSQL" node in the "Browser" panel.

![](img/p1/03.png)

### Creating the Database specifically for the Refuge Camp

In addition to the default database that PostgreSQL automatically generates, which is typically named 'postgres' or 'defaultdb' (in Aiven), it's important to create a separate, dedicated database specifically for our Refugee Camp workshop thematics.
...
And then we need to enable the PostGIS spatial extension, by executing the following SQL (as before / in the same window):
